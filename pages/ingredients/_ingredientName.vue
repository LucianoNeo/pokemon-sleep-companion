  ...
  <section class="mt-8">
    <div class="align-center d-flex justify-space-between">
      <div class="d-flex justify-center align-center">
        <v-btn plain @click="$router.push('/ingredients')">
          <v-icon>mdi-arrow-left</v-icon>
        </v-btn>
        <h3 class="text-uppercase zindex">{{ ingredient.name }}</h3>
      </div>
    </div>
    ...
    <div class="flex-column d-flex px-8 justify-center py-2 poison">
      <div class="d-flex align-center justify-center">
        <img :src="ingredient.img" :alt="ingredient.name" width="80" />
      </div>
    </div>
...
export default {
  async asyncData({ params }) {
    ...
    const ingredient = Object.values(ingredientsList).find((ingredient) => {
  ...
  data() {
    return {
      headers: [
        {
          text: 'Preço de venda (DreamShards)',
          value: 'sellValue',
          sortable: false,
          align: 'center',
        },
  ...
  computed: {
    filteredPokemonList() {
      return this.pokemonList.filter((pokemon) => {
        return pokemon.ingredients.some((filteredIngredient) => filteredIngredient === this.ingredient);
      })
    },
